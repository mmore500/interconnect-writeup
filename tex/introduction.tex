...
Digital evolution practitioners have a rich history of leveraging %parallel and 
...
Sentient Technologies also accelerated the deep learning training process by using many massively-parallel hardware accelerators (e.g., 100 GPUs) to evaluate the performance of candidate neural network architectures on image classification, language modeling, and image captioning problems
%In this case, hardware parallelism accelerated the deep learning training process used to evaluate individual solutions 
...
Parallelizing evaluation of a single %evolutionary
...
%accounted for during selection
%or deep learning weight update. %TODO tryna include lexicase
...
Distributed computing should also enable more computationally intensive %(potentially but not necessarily more complex) evolutionary 
or complex individuals.
%However, 
Developmental processes and emergent functionality necessitate dynamic interactions among components of an evolving individual. %emergent consolidated functionality?
...
% }
modeling them as a collection of discrete components configured through generative development (i.e., with indirect genetic representation) can promote %germane
...
%David Ackley has led toward such a distributed approach, laying out an ambitious vision for modular hardware at a theoretically unlimited scale \citep{ackley2011pursue} and demonstrating an algorithmic substrate for emergent agents that can 
...
While by no means certain, the idea that orders-of-magnitude increases in compute power will open up qualitatively different possibilities with respect to open-ended evolution is %not entirely unfounded.
well founded.
...
As with digital evolution, artificial neural networks (ANNs) were traditionally understood as a versatile, but auxiliary methodology --- both techniques were described %along the lines of
...
Similarly, progress toward realizing artificial life systems with %true
...
In addition to developing hardware-agnostic theory and methodology, we believe that pushing the envelope of open-ended evolution will analogously require %expressly
designing systems %to effectively
...
Contemporary scientific computing clusters lack key characteristics %highlighted
...
% Many natural systems --- such as ecosystems, genetic regulatory networks, and neural networks --- are known to exhibit small-world patterns of connectivity or interaction among components \citep{bassett2017small, fox2014herbivores, gaiteri2014beyond}.
...
% We anticipate that open-ended phenomena emerging across distributed hardware might also %involve
% use small-world connectivity dynamics.
% What would the impact be of providing a system of hierarchical log-time hardware interconnects as opposed to relying solely on local %hardware
% interconnects?

%In Supplementary Sections \ref{sec:proof1}, \ref{sec:proof2}, \ref{sec:proof3}, and \ref{sec:proof4}, we analyze the scaling relationship between system size and expected node-to-node hops traversed between computational elements interacting as part of an emergent small-world network \citep{Moreno_Ofria_2020}.
%\footnote{
...
%In Supplementary Section \ref{sec:proof4} we show that, with a specific traditional construction of small-world graphs, best-case mean hops per edge scales polynomially with graph size \citep{Moreno_Ofria_2020}.
...
%upcoming work incorporating genetic programming with
...
\subsection{Abstraction, Engineering, and Computational Scale}

Although designed with an eye toward scalability, largely along the lines outlined by Ackley, DISHTINY exchanges a uniform, evolutionary-passive substrate for manually-engineered %self-replicators.
...
More granular, less prescriptional approaches seem likely to become %ascendant
